...
// Accent colours per card so each one feels distinct
const CARD_ACCENTS = [
  { top: "#FF1E27", pattern: "rgba(255,30,39,0.18)", text: "#FF1E27" },
  { top: "#f97316", pattern: "rgba(249,115,22,0.18)", text: "#f97316" },
  { top: "#0ea5e9", pattern: "rgba(14,165,233,0.18)", text: "#0ea5e9" },
  { top: "#10b981", pattern: "rgba(16,185,129,0.18)", text: "#10b981" },
  { top: "#8b5cf6", pattern: "rgba(139,92,246,0.18)", text: "#8b5cf6" },
];

// ─── SVG decorative swirl pattern (inline, no external asset needed) ──────────
const SwirlPattern: React.FC<{ color: string }> = ({ color }) => (
  ...
  >
    {/* Repeating organic swirl paths — inspired by the reference card pattern */}
    {[...Array(6)].map((_, row) =>
      [...Array(5)].map((_, col) => {
        const cx = col * 90 - 20;
        const cy = row * 50 - 10;
        return (
          <g key={`${row}-${col}`} transform={`translate(${cx},${cy})`}>
            <path
              d={`M 10,25 C 10,10 30,5 40,15 C 50,25 45,45 30,45 C 15,45 5,35 10,25 Z`}
              fill="none"
              stroke={color}
              strokeWidth="1.5"
              strokeLinecap="round"
            />
            <path
              d={`M 18,28 C 18,20 28,17 34,22 C 40,27 37,38 30,38 C 23,38 18,34 18,28 Z`}
              fill="none"
              stroke={color}
              strokeWidth="1"
              opacity="0.6"
...
// ─── Business-card style ValueCard ────────────────────────────────────────────
const ValueCard: React.FC<{
  val: { title: string; desc: string; bg: string; border: string; icon: React.ReactNode };
  index: number;
  ...
  return (
    <div
      className="val-card shrink-0 flex flex-col relative"
      style={{
        width: cardWidth ?? "100%",
        height: 420,
        borderRadius: 24,
        ...
        // Layered card shadow — mimics the stacked-cards depth in the reference
        boxShadow: `
          0 2px 0 2px rgba(0,0,0,0.18),
          0 6px 0 4px rgba(0,0,0,0.12),
          0 28px 56px rgba(0,0,0,0.55),
          0 8px 24px rgba(0,0,0,0.3)
        `,
      }}
    >
      {/* ── TOP HALF: decorative patterned area ─────────────────────────── */}
      <div
        style={{
          flex: "0 0 55%",
          position: "relative",
      ...
      >
        ...
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: "radial-gradient(ellipse 70% 60% at 50% 110%, rgba(255,255,255,0.12) 0%, transparent 70%)",
            pointerEvents: "none",
          }}
        />

        {/* Icon circle — centred, prominent */}
        <div
          style={{
            position: "relative",
            zIndex: 2,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div
            style={{
              ...
              flexShrink: 0,
              boxShadow: `0 0 6px ${accent.top}80`,
            }}
          />
          <h4
            style={{
...
// ─── Desktop: GSAP-pinned horizontal sweep + vertical wave ───────────────────
const ValuesDesktopScroll: React.FC<{
  ...
  useLayoutEffect(() => {
    ...
    // Initial state — cards start rotated, shifted down, invisible
    ...
    const ctx = gsap.context(() => {
      const endXPercent = -(100 * (n - 1)) / n;
      const tl = gsap.timeline();

      // ① Horizontal rail sweep — the core left-to-right travel
      tl.to(track, { xPercent: endXPercent, ease: "none", duration: n }, 0);

      // ② Vertical wave — each card rises up from below as the rail brings it in
      //    stagger offset = 1 so each card gets its own scroll "beat"
      tl.to(cardEls, {
        y: 0,
        opacity: 1,
        scale: 1,
        ...
        ease: "power3.out",
        stagger: {
          each: 1,         // one card per full scroll "beat"
          from: "start",
        },
      }, 0);

      // ③ Secondary vertical bob — cards that have passed slightly drift upward
      //    creating a gentle floating-away sensation
      tl.to(cardEls, {
        y: -20,
        opacity: 0.55,
        duration: 0.5,
      ...
      }, n * 0.55); // starts playing mid-scroll so "used" cards visually recede
  ...
  return (
    <div
      ...
      style={{
        ...
        flexDirection: "column",
        justifyContent: "center",
        // Subtle dark bg during pin so cards read well
    ...
    >
      ...
      <div className="px-8 md:px-16 mb-10 shrink-0">
        <motion.span
          className="font-mono text-[10px] tracking-widest text-[#FF1E27] font-bold uppercase"
      ...
      >
        <div
          ...
          style={{
            ...
            alignItems: "center",   // vertically centred in row so Y motion is visible
          }}
        >
          {values.map((val, i) => (
            <ValueCard
              ...
              val={val}
              index={i}
              cardWidth="clamp(280px, 26vw, 360px)"
            />
          ))}
        </div>
      </div>
        ...
        {values.map((_, i) => (
          <div
            ...
            style={{
              ...
              width: 32,
              borderRadius: 100,
              background: "rgba(255,30,39,0.25)",
              position: "relative",
              overflow: "hidden",
            }}
          >
...
// ─── Mobile: vertical stacked cards with staggered whileInView ────────────────
const ValuesMobileStack: React.FC<{
  values: Array<{ title: string; desc: string; bg: string; border: string; icon: React.ReactNode }>;
}> = ({ values }) => (
...
const ValuesHorizontalScroll: React.FC<{
  ...
  return isMobile
    ? <ValuesMobileStack values={values} />
    : <ValuesDesktopScroll values={values} />;
};
...
export const BrandEcosystem: React.FC<{ onPlayDemo?: () => void }> = ({ onPlayDemo }) => {
  ...
  const subBrands: SubBrand[] = [
    {
      ...
      purpose:
        "እንቆቅCash (EnqoqCash) is a regulated, multi-platform, knowledge-driven trivia network designed to connect and reward intellect.",
    ...
    },
    {
      ...
      pillars: [
        ...
        { title: "Innovation", desc: "Exploring new streaming technologies and immersive user experiences." },
        { title: "User-Centric", desc: "Tailored experiences crafted around user preferences and live feedback." },
      ],
    },
    {
      id: "etop",
      ...
      pillars: [
        { title: "Creativity", desc: "Bold, innovative, original content structure that captivates communities." },
        { title: "Collaboration", desc: "Fusing diverse talents to produce world-class premium multimedia." },
    ...
    },
    {
      ...
      purpose:
        "To serve as the go-to information hub for all things multimedia — offering insights, news, and analysis to help creators, businesses, and enthusiasts stay ahead.",
      icon: <BookOpen className="w-6 h-6 text-white" />,
      color: "#10b981",
      bgGrad: "from-teal-500 to-emerald-600",
      pillars: [
        { title: "Accuracy", desc: "Well-researched, reliable, and continuously up-to-date industry insights." },
  ...
  return (
    ...
    >
      ...
      {/* Values section — full-bleed responsive */}
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${
                          isActive ? "text-white" : "bg-zinc-100 text-zinc-400 group-hover:bg-zinc-200"
                        }`}
            ...
            >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 group/deck">
                      {activeBrand.pillars.map((pil, idx) => (
                        <motion.div
                          ...
                          className="relative overflow-hidden bg-white border border-neutral-200/60 dark:border-white/10 p-5 rounded-2xl flex flex-col justify-between transition-all duration-300 group/item cursor-pointer min-h-[140px] group-hover/deck:opacity-60 hover:!opacity-100"
                        >
                          <div
                            className="absolute left-0 top-0 bottom-0 w-[4px] rounded-l-2xl opacity-60 group-hover/item:opacity-100 transition-all duration-300"
                            style={{ backgroundColor: activeBrand.color }}
                          />
                          <div
                          ...
                          <div className="space-y-2 relative z-10">
                            ...
                            <h4 className="font-serif italic text-[16px] text-zinc-900 dark:text-neutral-100 group-hover/item:text-[#FF1E27] font-semibold tracking-tight transition-colors">
                              {pil.title}
                            </h4>
                          </div>
                          <p className="text-zinc-500 dark:text-zinc-400 text-xs leading-relaxed font-light pt-2.5 border-t border-dashed border-neutral-200/50 dark:border-white/5 relative z-10">
                            {pil.desc}
                          </p>
                        </motion.div>
                      ))}
              ...
              <div className="pt-8 border-t border-zinc-150 mt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                {activeBrand.id === "enqoq" ? (
                  <button
                    onClick={onPlayDemo}
                    className="flex items-center gap-2 bg-[#FF1E27] hover:bg-[#C90E16] text-white font-display font-medium text-xs px-5 py-3 rounded-lg transition-all cursor-pointer shadow-md"
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
            ...
            >
              ...
              <p className="text-zinc-400 text-sm leading-relaxed font-light pt-2 max-w-sm">
                We unify five integrated sub-brands into one cohesive loop — facilitating complete content design, worldwide streaming, modern training, and news delivery.
              </p>
            </motion.div>
